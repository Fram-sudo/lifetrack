#!/usr/bin/env python3
"""Parser complet SG + Revolut → JSON unifié (avec tous les correctifs)"""
import re, json
...
SG_DIR  = "/path/to/your/sg/pdfs"
REV_PDF = "/path/to/your/revolut/statement.pdf"
LOAN_AMOUNTS = {74.52, 78.87, 75.00, 74.00, 79.00}
...
MERCHANT_MAP = [
    ...
    (r'(?i)FOURNILMARIE', 'Fournil Marie'),
    (r'(?i)LA\s*MIE\s*C[AÂ]LINE', 'La Mie Câline'),
    (r'(?i)STATION\s*GOURM|LASTATIONGOURM', 'La Station Gourmande'),
    (r'(?i)(?:NYA\*SARL\s*)?3\s*BDA\s*EXPRESSO|3BDAEXPRESSO', '3Bda Expresso'),
    (r'(?i)BERGOULI|SAS\s*BERGOULI', 'Bergouli'),
    (r'(?i)JULES\s*(?:&|ET)?\s*JOH?N', 'Jules & John'),
    (r'(?i)COLUMBUS\s*CAF', 'Columbus Café & Co'),
    (r'(?i)NEWSCHOOLTACOS', 'New School Tacos'),
    (r'(?i)PTIT\s*RIZTO|LEPTITRIZTO', 'Le Ptit Rizto'),
    (r'(?i)AMIEDESPAINS', "L'Amie des Pains"),
    (r'(?i)ANGE\s*BOUL|ANGEBOUL', 'Boulangerie Ange'),
    (r'(?i)WIISMILE', 'Wiismile'),
    (r'(?i)LACQUEMANT', "Pâtisserie Lacquemant"),
    (r'(?i)FEUILLEDOR', "Feuille d'Or"),
    (r'(?i)GREENSHEEP|GREEN\s*SHEEP', 'Green Sheep'),
    (r'(?i)METSGUSTO', 'Mets & Gusto'),
    (r'(?i)LYKIMSAN', 'Lykimsan'),
    (r'(?i)MASALA', 'Masala'),
    (r'(?i)HOAMAI', 'Hoa Mai'),
    (r'(?i)ALSACENEKO', 'AlsaceNeko'),
    (r'(?i)RISECORP', 'Risecorp'),
    (r'(?i)\bSNCF\b', 'SNCF'), (r'(?i)\bRATP\b', 'RATP'),
    (r'(?i)MODALIS', 'Modalis'), (r'(?i)BLABLACAR', 'BlaBlaCar'),
    (r'(?i)TRANSAVIA', 'Transavia'), (r'(?i)EASYJET', 'EasyJet'),
    (r'(?i)RYANAIR', 'Ryanair'), (r'(?i)AIR\s*FRANCE', 'Air France'),
    (r'(?i)UBER\s*\*?\s*TRIP|^Uber$', 'Uber'),
    (r'(?i)SEMACO', 'SEMACO'), (r'(?i)IDELIS', 'Idelis'),
    (r'(?i)IPADOUR', 'Ipadour'), (r'(?i)VELOSTATION|VELO\s*STATION', 'Velo Station'),
    (r'(?i)LAFENETRETAXI', 'La Fenêtre Taxi'),
    (r'(?i)LAV[- ]?MATIC', 'Lav-Matic'),
    ...
    (r'(?i)BETABLOC|BETA[- ]BLOC', 'Beta Bloc Escalade'),
    ...
    (r'(?i)GARANTME', 'Garantme'),
    (r'(?i)PHARMAC', 'Pharmacie'),
    (r'(?i)MUTUELLE|LMDE|INTERIALE', 'Mutuelle'),
    (r'(?i)LES\s*BELLES\s*ANN|LESBELLESANNEES', 'Les Belles Années'),
    (r'(?i)\bALPIQ\b', 'Alpiq'),
    (r'(?i)KC[- ]PAU', 'KC-PAU'),
    (r'(?i)STREET\s*HOUSE\s*BARBER|STREETHOUSE', 'Street House Barber'),
    (r'(?i)\bLAHPROD\b', 'Lahprod'),
    (r'(?i)PAYPAL', 'PayPal'),
    ...
    (r'(?i)FOUNDEV\w*FRANCE', 'Foundever France'),
    (r'(?i)DRFIP', 'DRFIP Nouvelle-Aquitaine'),
    (r'(?i)CROUS', 'CROUS'),
    (r'(?i)PAUBDC', 'PAU BDC'),
    (r'(?i)IZLYSMONEY', 'IzlysMoney'),
    (r'(?i)IZLY', 'Izly'),
    (r'(?i)LAPOSTE', 'La Poste'),
    (r'(?i)LALUDIKAVERN', 'La Ludikaverne'),
    (r'(?i)EPICNPC', 'Epic NPC'),
    (r'(?i)SPHOLYENERGY|SPH\s*OLY', 'SPH Oly Energy'),
    (r'(?i)IRUN', 'I-Run'),
    (r'(?i)TGSPAU', 'TGS Pau'),
    (r'(?i)PAUAVDEL', "PAU Av. de l'Université"),
    (r'(?i)CLOUSPAU', 'Clous Pau'),
    (r'(?i)STAP|S\.T\.A\.P', 'STAP'),
]
...
def categorize(label, raw_ctx, is_credit, amount=0.0, user_rules=None):
    ...
    # ── Transferts internes SG ↔ Revolut ──
    revolut_topup = bool(re.search(r'\brevolut\s+\d{3,4}\b', raw_ctx, re.I)) or \
                    bool(re.search(r'Revolut\s*\*{0,2}\d{3,4}\*?', raw_ctx))
    if any(x in ctx for x in ['REVOFRP2','BQREVOCPT889','88902022','28233000018890']) or revolut_topup:
    ...
    # ── Prêt ──
    if round(abs(amount), 2) in LOAN_AMOUNTS and not is_credit:
        return ('🏦 Remboursement prêt', 'dépense')

    # ── Virements émis SG→SG (virements européens LOGITEL) ──
    if re.search(r'(?:000001)?VIREUROPE', ctx) and not any(x in ctx for x in ['REVOLUT','REVOFRP2','BQREVOCPT']):
    ...
    if is_credit:
        ...
        if any(x in ctx for x in ['BOURSE','CROUS','DRFIP','ACADEMIE','ENSEIGNEMENTSUP']):
            return ('🎓 Bourse & Aides sociales', 'revenu')
        if any(x in ctx for x in ['SALAIRE','FOUNDEVERFRANCE','FOUNDEVER','PAYE','TRAITEMENT']):
    ...
    # ── Dépenses ──
    if any(x in ctx for x in ['LESBELLESANNEES','BELLES']):
        return ('🏠 Loyer & Charges', 'dépense')
    if any(x in ctx for x in ['ALPIQ','EDF','ENGIE','SPHOLYENERGY']):
        return ('🏠 Loyer & Charges', 'dépense')
    if any(x in ctx for x in ['KCPAU','KCSAU']):
        return ('📺 Abonnements', 'dépense')
    if any(x in ctx for x in ['LMDE','INTERIALE','MUTUELLE','PHARMAC','DRWER']):
        return ('🏥 Santé', 'dépense')
    if any(x in ctx for x in ['NETFLIX','SPOTIFY','CRUNCHYROLL','SOBRIO','LASTPASS','CYBERGHOST',
                                'CLAUDE','ANTHROPIC','APPLE','UBERONE','AMAZONPRIME']):
        return ('📺 Abonnements', 'dépense')
    if any(x in ctx for x in ['SNCF','RATP','MODALIS','BLABLACAR','FLIXBUS','TRANSAVIA',
                                'EASYJET','RYANAIR','AIRFRANCE','SEMACO','IDELIS','IPADOUR',
                                'LAFENETRETAXI','VELOSTATION']):
        return ('🚗 Transport', 'dépense')
    if any(x in ctx for x in ['LECLERC','CARREFOUR','LIDL','ALDI','INTERMARCHE','MONOPRIX',
                                'SUPERU','AUCHAN','FOURNILMARIE','UBEREATS','DELIVEROO',
                                'MCDONALD','KFC','BURGERKING','NEWSCHOOLTACOS','PTITRIZTO',
                                'BERGOULI','AMIEDESPAINS','3BDAEXPRESSO','LASTATIONGOURM',
                                'ANGEBOUL','METSGUSTO','HOAMAI','WIISMILE','LACQUEMANT',
                                'MASALA','FEUILLEDOR','GREENSHEEP','LYKIMSAN','ESPRITDESHALLES',
                                'STAP','PAUBDC','SARL37','SAS37','KAMOK','RONDEDESPAINS',
                                'GRANGE','LAMIECA','BABETTE','JULESJOHN','COLUMBUS','WINFIELD',
                                'SODEXO','BOUCHERIEMIDI','NEWREST','ALSACENEKO','RISECORP',
                                'CLOUSPAU']):
        return ('🛒 Alimentation', 'dépense')
    if any(x in ctx for x in ['RETRAITDAB','RETRAITGAB']):
    ...
    if any(x in ctx for x in ['GOOGLEPLAY','XSOLLA','INSTANTGAMING','STEAMPURCHASE','STEAM',
                                ...
                                'SONYINTERACT','BETABLOC','LALUDIKAVERN','G2G','ZZREROLL',
                                'CGR','PATHE','UGC']):
        return ('🎮 Loisirs & Jeux', 'dépense')
    if any(x in ctx for x in ['AMAZON','VINTED','MGP','BACKMARKET','LDLC','RHINOSHIELD',
                                'IRUN','LAPOSTEBOUTIQ','EPICNPC','PAYPAL','SPAIRUP']):
        return ('🛍️ Shopping', 'dépense')
    if any(x in ctx for x in ['STREETHOUSEBARBER','STREETHOUSE','TREATWELL','LAHPROD','TGSPAU']):
...
def build_sg_label(desc, cont_lines):
    ...
    if re.match(r'AVANTAGE', d, re.I):
        return 'Avantage commercial Sobrio'
...
def main():
    ...
    sg_pdfs = sorted(p for p in Path(SG_DIR).glob("ReleveCompte_*.pdf") if '20190806' not in p.name)
    print(f"SG PDFs: {len(sg_pdfs)}")
    for pdf in sg_pdfs:
        for tx in parse_sg_pdf(str(pdf)):
            by_year.setdefault(tx['date'][:4], []).append(tx)

    sg_totals = {y: len(v) for y, v in sorted(by_year.items())}
    print(f"SG by year: {sg_totals}  total={sum(sg_totals.values())}")

    rev_txs = parse_revolut_pdf(REV_PDF)
    print(f"Revolut: {len(rev_txs)}")
    for tx in rev_txs:
        by_year.setdefault(tx['date'][:4], []).append(tx)

    for y in by_year:
        by_year[y].sort(key=lambda t: t['date'])

    totals = {y: len(v) for y, v in sorted(by_year.items())}
    print(f"Combined by year: {totals}  total={sum(totals.values())}")

    with open('/tmp/transactions_all.json', 'w', encoding='utf-8') as f:
        json.dump({'by_year': by_year}, f, ensure_ascii=False, indent=2)
    print("✓ Saved /tmp/transactions_all.json")
