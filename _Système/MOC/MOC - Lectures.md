...
```dataviewjs
const TYPES_CFG = {
  "manga":  { type:"manga",  icon:"📖", label:"Manga",        color:"#8878c3", bg:"rgba(136,120,195,0.12)",
    statuts_finis:["terminé","lu"], statut_voir:"à lire", label_voir:"À lire", label_finis:"Lus",
    placeholder:"🔍 Rechercher un manga…" },
  "manwha": { type:"manwha", icon:"📗", label:"Manwha",       color:"#5f8dd3", bg:"rgba(95,141,211,0.12)",
    statuts_finis:["terminé","lu"], statut_voir:"à lire", label_voir:"À lire", label_finis:"Lus",
    placeholder:"🔍 Rechercher un manwha…" },
  "manhua": { type:"manhua", icon:"📘", label:"Manhua",       color:"#6a9fd8", bg:"rgba(106,159,216,0.12)",
    statuts_finis:["terminé","lu"], statut_voir:"à lire", label_voir:"À lire", label_finis:"Lus",
    placeholder:"🔍 Rechercher un manhua…" },
  "livre":  { type:"livre",  icon:"📚", label:"Roman & Livre", color:"#4e8a5a", bg:"rgba(78,138,90,0.12)",
    statuts_finis:["terminé","lu"], statut_voir:"à lire", label_voir:"À lire", label_finis:"Lus",
    placeholder:"🔍 Rechercher un livre…" },
}

if (!window._mocLectures) window._mocLectures = { type: "manga" }

const allPages = {
  "manga":  dv.pages('"2 - Domaines/Médias"').where(p => p.type === "manga").array(),
  "manwha": dv.pages('"2 - Domaines/Médias"').where(p => p.type === "manwha").array(),
  "manhua": dv.pages('"2 - Domaines/Médias"').where(p => p.type === "manhua").array(),
  "livre":  dv.pages('"2 - Domaines/Médias"').where(p => p.type === "livre").array(),
}

let activeFilter = "tout"
let activeSearch  = ""

const activeType = () => window._mocLectures.type
const cfg        = () => TYPES_CFG[activeType()]
const getPages   = () => allPages[activeType()] || []

const wrap = dv.container.createDiv()

// --- STATS BAR ---
const barWrap = wrap.createDiv()
...
const renderBar = () => {
  ...
  const aLire   = pages.filter(p => ["à lire","backlog"].includes(p.statut)).length
  const moyNote = (() => {
  ...
  for (const s of [
    ...
    { val:aLire,        label:c.label_voir,   bg:"rgba(196,148,58,0.10)",  color:"#c4943a" },
...
toggleRow.style.cssText = "display:flex;gap:0;margin-bottom:20px;border-radius:8px;overflow:hidden;border:1px solid var(--background-modifier-border);width:fit-content;flex-wrap:wrap;"

const toggleBtns = {}
for (const [key, c] of Object.entries(TYPES_CFG)) {
  const btn = toggleRow.createEl("button")
  btn.style.cssText = "padding:7px 18px;border:none;cursor:pointer;font-size:0.88em;font-weight:700;font-family:inherit;transition:background 0.15s,color 0.15s;"
  btn.innerHTML = `${c.icon} ${c.label}`
  toggleBtns[key] = btn
  btn.onclick = () => {
    window._mocLectures.type = key
    activeFilter = "tout"
    activeSearch  = ""
    searchInput.value = ""
...
const FILTERS = [
  ...
  { label:()=>cfg().label_voir,  key:"a_lire"   },
...
function makeCard(grid, page) {
  ...
  if (page.statut) {
    const STATUS = {
      ...
      "à lire":    { bg:"rgba(196,148,58,0.92)",   txt:"◯ À lire"    },
  ...
  let sub = ""
  const t = activeType()
  if ((t === "manga" || t === "manwha" || t === "manhua") && page.chapitres_lus) {
    sub = `Ch. ${page.chapitres_lus}`
  } else if (t === "livre" && page.pages_lues && page.pages) {
    sub = `${page.pages_lues} / ${page.pages} p.`
  }
...
const renderGrid = () => {
  ...
  if (activeFilter === "tout") {
    ...
    const aLireList = sf.filter(p => ["à lire","backlog"].includes(p.statut))
    if (aLireList.length) {
      contentDiv.createEl("h2").textContent = `◯ ${c.label_voir}`
      const grid = contentDiv.createDiv(); grid.style.cssText = GRID_STYLE
      for (const p of aLireList) makeCard(grid, p)
    }
  ...
  else if (activeFilter === "a_lire")   filtered = sf.filter(p => ["à lire","backlog"].includes(p.statut))
