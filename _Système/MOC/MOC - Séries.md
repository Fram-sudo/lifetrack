...
```dataviewjs
const CFG = { icon:"📺", label:"Série", color:"#df8e1d", bg:"rgba(223,142,29,0.12)" }
const all = dv.pages('"2 - Domaines/Médias/Séries"').where(p => p.type === "série")
const enCours  = all.where(p => p.statut === "en cours").length
const termines = all.where(p => p.statut === "terminé" || p.statut === "vu").length
const aVoir    = all.where(p => p.statut === "à voir").length
const enPause  = all.where(p => p.statut === "en pause").length
...
for (const c of [
  { val:all.length,  label:"total",      bg:"rgba(223,142,29,0.10)", color:"#df8e1d" },
  { val:enCours,     label:"en cours",   bg:"rgba(30,102,245,0.10)", color:"#1e66f5" },
  { val:termines,    label:"terminées",  bg:"rgba(78,138,90,0.10)",  color:"#4e8a5a" },
  { val:enPause,     label:"en pause",   bg:"rgba(136,120,195,0.10)",color:"#8878c3" },
  { val:aVoir,       label:"à voir",     bg:"rgba(122,118,110,0.10)",color:"#7a766e" },
...
function showPlaceholder(box) {
  ...
  ph.createDiv().style.cssText = "font-size:2.8em;line-height:1;"; ph.firstChild.textContent = CFG.icon
  const lbl = ph.createDiv(); lbl.style.cssText = `font-size:0.65em;font-weight:800;letter-spacing:0.08em;color:${CFG.color};text-transform:uppercase;opacity:0.8;`; lbl.textContent = CFG.label
}
...
function makeCard(grid, page) {
  ...
  if (page.statut) {
    const s = STATUS_MAP[page.statut.toLowerCase()]
    if (s) { const badge = imgBox.createDiv(); badge.style.cssText = `position:absolute;bottom:6px;left:6px;padding:2px 7px;border-radius:12px;font-size:0.67em;font-weight:700;color:#fff;background:${s.bg};letter-spacing:0.03em;`; badge.textContent = s.txt }
  }
  const info = card.createDiv(); info.style.cssText = "padding:9px 10px;flex:1;display:flex;flex-direction:column;gap:3px;"
  if (page.note) { const stars = info.createDiv(); stars.style.cssText = "font-size:0.78em;color:#c4943a;"; const full=Math.round(page.note/2); stars.textContent="★".repeat(full)+"☆".repeat(5-full)+"  "+page.note+"/10" }
...
const GRID = "display:grid;grid-template-columns:repeat(auto-fill,minmax(145px,1fr));gap:14px;margin:12px 0 28px;"
const FILTERS = [
  {label:"Tout",key:"tout"},{label:"En cours",key:"en_cours"},{label:"Terminées",key:"termines"},
  {label:"À voir",key:"a_voir"},{label:"En pause",key:"pause"},{label:"Abandonné",key:"abandonne"},{label:"Notes",key:"notes"},
]
let activeFilter="tout", activeSearch=""
...
searchInput.type="search"; searchInput.placeholder="🔍 Rechercher une série…"
...
const tabBar=wrap.createDiv(); tabBar.style.cssText="display:flex;gap:6px;flex-wrap:wrap;margin-bottom:20px;"
const contentDiv=wrap.createDiv()
const tabRefs=[]
for(const f of FILTERS){
  const btn=tabBar.createEl("button")
  const refresh=()=>{btn.style.cssText=activeFilter===f.key?"padding:5px 14px;border-radius:20px;border:none;background:var(--interactive-accent);color:#fff;cursor:pointer;font-size:0.84em;font-weight:700;font-family:inherit;":"padding:5px 14px;border-radius:20px;border:1px solid var(--background-modifier-border);background:var(--background-secondary);color:var(--text-muted);cursor:pointer;font-size:0.84em;font-family:inherit;";btn.textContent=f.label}
  btn.onclick=()=>{activeFilter=f.key;tabRefs.forEach(r=>r());renderGrid()}
  refresh();tabRefs.push(refresh)
}
const renderGrid=()=>{
  contentDiv.empty()
  const sf=activeSearch?allSeries.filter(p=>(p.titre||p.file.name).toLowerCase().includes(activeSearch)):allSeries
  if(activeFilter==="tout"){
    const groups=[
      {title:"🔥 En cours",   items:sf.filter(p=>p.statut==="en cours").sort((a,b)=>b.file.mtime-a.file.mtime)},
      {title:"✓ Terminées",   items:sf.filter(p=>p.statut==="terminé"||p.statut==="vu").sort((a,b)=>b.file.mtime-a.file.mtime)},
      {title:"⏸ En pause",   items:sf.filter(p=>p.statut==="en pause")},
      {title:"◯ À voir",      items:sf.filter(p=>p.statut==="à voir")},
      {title:"✕ Abandonnées", items:sf.filter(p=>p.statut==="abandonné")},
    ]
    for(const g of groups){if(g.items.length){contentDiv.createEl("h2").textContent=g.title;const grid=contentDiv.createDiv();grid.style.cssText=GRID;for(const p of g.items)makeCard(grid,p)}}
    if(!sf.length) contentDiv.createEl("p",{attr:{style:"color:var(--text-muted);font-style:italic;font-size:0.88em;"}}).textContent="Aucune série dans le vault."
    return
  }
  const map={en_cours:p=>p.statut==="en cours",termines:p=>p.statut==="terminé"||p.statut==="vu",a_voir:p=>p.statut==="à voir",pause:p=>p.statut==="en pause",abandonne:p=>p.statut==="abandonné",notes:p=>p.note}
  const filtered=(map[activeFilter]?sf.filter(map[activeFilter]):sf).sort((a,b)=>activeFilter==="notes"?(b.note||0)-(a.note||0):0)
  if(!filtered.length){contentDiv.createEl("p",{attr:{style:"color:var(--text-muted);font-style:italic;font-size:0.88em;"}}).textContent="Aucun contenu.";return}
  const grid=contentDiv.createDiv();grid.style.cssText=GRID;for(const p of filtered)makeCard(grid,p)
}
renderGrid()
```
