<%*
const type_lecture = await tp.system.suggester(
  ["📖 Manga", "📗 Manwha", "📘 Manhua"],
  ["manga", "manwha", "manhua"]
);
...
const _mg_sub = {"à lire":"À lire","en cours":"En cours","lu":"Terminé","abandonné":"Abandonné"}[statut] || "À lire"
await tp.file.move("2 - Domaines/Médias/Manga & Manwha/" + _mg_sub + "/" + tp.file.title)
-%>
---
type: "<% type_lecture %>"
...
const MOVE_MAP = {"à lire":"2 - Domaines/Médias/Manga & Manwha/À lire","en cours":"2 - Domaines/Médias/Manga & Manwha/En cours","lu":"2 - Domaines/Médias/Manga & Manwha/Terminé","abandonné":"2 - Domaines/Médias/Manga & Manwha/Abandonné"}
;(function(){
...
mkRow("Lu",     (p.chapitres_lus || 0) + " ch.",  () => editField("chapitres_lus", "Chapitres lus", p.chapitres_lus, true))
mkRow("Total",  p.chapitres_total ? p.chapitres_total + " ch." : "-", () => editField("chapitres_total", "Chapitres total", p.chapitres_total || "", true))
...
const tomeCovers = covers.slice(1)
if (tomeCovers.length > 0) {
  const thumbRow = this.container.createEl("div", {attr:{style:"display:flex;gap:10px;flex-wrap:wrap;margin-bottom:16px;"}})
  for (let i = 0; i < tomeCovers.length; i++) {
    const src = getSrc(tomeCovers[i])
...
btnSess.onclick = () => {
  ...
  showForm("Ajouter une session", {
    ...
    ch_deb: {label:"Chapitre début", value:"", type:"number"},
    ch_fin: {label:"Chapitre fin (facultatif)", value:"", type:"number"},
  }, async ({date, ch_deb, ch_fin}) => {
    if (!date || !ch_deb) return
    const ch_d = parseInt(ch_deb) || null
    const ch_f = parseInt(ch_fin || ch_deb) || ch_d
    const sess = {date}
    sess["ch_debut"] = ch_d
    sess["ch_fin"]   = ch_f
    await app.fileManager.processFrontMatter(file, fm => {
      ...
      const total = fm.sessions.reduce((t, s) => {
        const d = s["ch_debut"] != null ? s["ch_debut"] : null
        const a = s["ch_fin"]   != null ? s["ch_fin"]   : d
        return t + ((d != null && a != null) ? a - d + 1 : (Number(s.ch) || 0))
      }, 0)
      fm["chapitres_lus"] = total
      if (ch_f) fm["dernier_chapitre"] = ch_f
    })
...
btnMod.onclick = e => {
  ...
  menu.addItem(it => { it.setTitle("✏ Moments marquants"); it.onClick(() => editBodySection("Moments marquants", "marquants")) })
  menu.addItem(it => { it.setTitle("✏ Personnages clés"); it.onClick(() => editBodySection("Personnages clés", "personnages")) })
  ...
  for (let i = 1; i < curC.length; i++) {
    const _i = i
    menu.addItem(it => {
      it.setTitle("📚 Tome " + String(_i).padStart(2,"0") + (curC[_i] ? " ✓" : " (vide)"))
      it.onClick(() => editCover(_i, "Tome " + String(_i).padStart(2,"0"), curC))
    })
  }
  menu.addItem(it => { it.setTitle("➕ Ajouter une cover de tome"); it.onClick(() => {
    const idx = Math.max(1, curC.length)
    editCover(idx, "Tome " + String(idx).padStart(2,"0"), curC)
  })})
  menu.showAtMouseEvent(e)
}
const btnType = actBar.createEl("button", {attr:{style:BTN}})
btnType.textContent = "🔄 Type"
btnType.onclick = e => {
  const menu = new Menu()
  ;["manga","manwha","manhua","livre"].forEach(_t=>{ menu.addItem(it=>{ const _lbl=_t==="livre"?"Roman & Livre":_t.charAt(0).toUpperCase()+_t.slice(1); it.setTitle((_t===p.type?"✓ ":"   ")+_lbl); if(_t!==p.type) it.onClick(async()=>{ await app.fileManager.processFrontMatter(file, fm=>{ fm.type=_t; if(Array.isArray(fm.tags)){fm.tags=fm.tags.filter(g=>!["manga","manwha","manhua","livre"].includes(g)); fm.tags.push(_t);} }); new Notice("Type → "+_lbl, 2000) }) }) })
  menu.showAtMouseEvent(e)
}
```
...
const totalManga = () => sessions.reduce((t, s) => {
  const de = s["ch_debut"] ?? s["ch_début"] ?? null, a = s["ch_fin"] ?? de
  return t + (de != null && a != null ? a - de + 1 : Number(s.ch) || 0)
}, 0)
...
const renderManga = () => {
  ...
  for (const h of ["Date","Chapitres","Nb",""]) {
  ...
  for (const {s, i} of sorted) {
    ...
    const de = s["ch_debut"] ?? s["ch_début"] ?? null, a = s["ch_fin"] ?? de
    const count = de != null && a != null ? a - de + 1 : (s.ch ?? "?")
    const plage = de == null ? "-" : de === a ? "ch. " + de : "ch. " + de + " → " + a
    ...
    tr.createEl("td", {attr:{style:TD + "color:var(--text-muted);"}}).textContent = typeof count === "number" ? count + " ch." : count
    ...
    editBtn.onclick = () => showSessionForm("Modifier la session", {
      ...
      ch_deb: {label:"Chapitre début", value:de ?? "", type:"number"},
      ch_fin: {label:"Chapitre fin", value:a ?? "", type:"number"},
    }, async ({date:d2, ch_deb, ch_fin}) => {
      if (!d2) return
      sessions[i] = {date:d2, ch_debut:parseInt(ch_deb)||null, ch_fin:parseInt(ch_fin||ch_deb)||null}
      await app.fileManager.processFrontMatter(file, fm => { fm.sessions = sessions })
      new Notice("Session modifiée !", 2000); renderManga()
    })
    ...
    delBtn.onclick = async () => {
      ...
      await app.fileManager.processFrontMatter(file, fm => { fm.sessions = sessions })
      new Notice("Session supprimée.", 2000); renderManga()
    }
  }
  const tot = totalManga()
  tableWrap.createEl("p", {attr:{style:"font-size:0.82em;color:var(--text-muted);margin-top:6px;"}}).innerHTML = "<strong>" + tot + "</strong> chapitre" + (tot > 1 ? "s" : "") + " au total"
}
renderManga()
```
