...
const statut = await tp.system.suggester(
  ["📋 Backlog", "▶️ En cours", "✅ Terminé", "❌ Abandonné"],
  ["backlog", "en cours", "terminé", "abandonné"]
);
const _je_sub = {"backlog":"Backlog","en cours":"En cours","terminé":"Terminé","abandonné":"Abandonné"}[statut] || "Backlog"
await tp.file.move("2 - Domaines/Médias/Jeux Vidéo/" + _je_sub + "/" + tp.file.title)
...
const MOVE_MAP = {"backlog":"2 - Domaines/Médias/Jeux Vidéo/Backlog","en cours":"2 - Domaines/Médias/Jeux Vidéo/En cours","terminé":"2 - Domaines/Médias/Jeux Vidéo/Terminé","abandonné":"2 - Domaines/Médias/Jeux Vidéo/Abandonné"}
;(function(){
...
const STATUS_COLOR = {"backlog":"#7a766e","en cours":"#1e66f5","termine":"#40a02b","abandonne":"#d20f39"}
const STATUS_LIST  = ["backlog","en cours","terminé","abandonné"]
const STATUS_KEY   = {"backlog":"backlog","en cours":"en cours","terminé":"termine","abandonné":"abandonne"}
...
const mkRow = (label, display, onEdit) => {
  ...
  r.createEl("span", {attr:{style:"color:var(--text-muted);width:75px;flex-shrink:0;"}}).textContent = label
...
sRow.createEl("span", {attr:{style:"color:var(--text-muted);width:75px;flex-shrink:0;"}}).textContent = "Statut"
...
const _pl = (v => { if (!v || v === "") return ""; if (typeof v === "string") return v; try { return [...v].filter(Boolean).join(", ") } catch(e) { return "" } })(p.plateforme)
mkRow("Plateforme", _pl || "-", () => editField("plateforme", "Plateforme", _pl, false))
...
if (p.opus > 1) mkRow("Opus", String(p.opus) + " jeux", () => editField("opus", "Nombre de jeux (série)", p.opus, true))
mkRow("Temps",      (() => { const tot = p["heures_jouées"] || 0; const hh = Math.floor(tot), mm = Math.round((tot - hh) * 60); return mm ? `${hh}h${String(mm).padStart(2,"0")} jouées` : `${hh}h jouées` })(), null)

const nRow = infoCol.createEl("div", {attr:{style:"display:flex;gap:10px;font-size:0.85em;align-items:center;"}})
nRow.createEl("span", {attr:{style:"color:var(--text-muted);width:75px;flex-shrink:0;"}}).textContent = "Note"
...
const nb = Math.max(p.opus || 1, covers.length)
if (nb > 1) {
  ...
  for (let i = 0; i < nb; i++) {
    ...
    col.createEl("span", {attr:{style:"font-size:0.72em;color:var(--text-muted);font-weight:600;"}}).textContent = "Opus " + String(i+1).padStart(2,"0")
  }
}
...
btnSess.onclick = () => {
  ...
  showForm("Ajouter une session", {
    ...
    h:    {label:"Heures jouées (ex: 1:30)", value:"1:00", type:"text"},
  }, async ({date, h}) => {
    if (!date) return
    const parseH = v => {
    ...
    await app.fileManager.processFrontMatter(file, fm => {
      ...
      fm["heures_jouées"] = (fm.sessions || []).reduce((t, s) => t + parseH(s.h), 0)
    })
...
btnMod.onclick = e => {
  const menu = new Menu()
  menu.addItem(it => { it.setTitle("✏ Description"); it.onClick(() => editBodySection("Description", "description")) })
  ...
  while (curC.length < (p.opus || 0)) curC.push("")
  curC.forEach((cv, idx) => menu.addItem(it => {
    it.setTitle("🖼 Opus " + String(idx+1).padStart(2,"0") + (cv ? " ✓" : " (vide)"))
    it.onClick(() => showForm("Modifier - Opus " + String(idx+1).padStart(2,"0"),
...
const parseH = v => { const s = String(v||"").trim(); if(s.includes(":")) { const [h,m]=s.split(":").map(Number); return (h||0)+(m||0)/60 }; return parseFloat(s)||0 }
const fmtH = h => { const hh=Math.floor(h), mm=Math.round((h-hh)*60); return mm ? `${hh}h${String(mm).padStart(2,"0")}` : `${hh}h` }
...
const renderJeu = () => {
  ...
  for (const {s, i} of sorted) {
    ...
    editBtn.onclick = () => showSessionForm("Modifier la session", {
      ...
      h:    {label:"Durée (ex: 1:30)", value:String(s.h||""), type:"text"},
    }, async ({date:d2, h:h2}) => {
      if (!d2) return
      sessions[i] = {date:d2, h:h2}
      await app.fileManager.processFrontMatter(file, fm => {
        fm.sessions = sessions
        fm["heures_jouées"] = sessions.reduce((t, s) => t + parseH(s.h), 0)
      })
      new Notice("Session modifiée !", 2000); renderJeu()
    })
    ...
    delBtn.onclick = async () => {
      ...
      new Notice("Session supprimée.", 2000); renderJeu()
    }
  }
  const tot = sessions.reduce((t, s) => t + parseH(s.h), 0)
  tableWrap.createEl("p", {attr:{style:"font-size:0.82em;color:var(--text-muted);margin-top:6px;"}}).innerHTML = "<strong>" + fmtH(tot) + "</strong> jouées au total"
}
renderJeu()
```
